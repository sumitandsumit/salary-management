...
## 2026-09-22 — FE polish, icons, table fixes
...
- Outcome: replaced `@tabler/icons-react` with inline inline SVGs (Pencil, +%, Offboard/Activate);
  fixed table column widths (Name 13%, Email 20%, Dept 14%, Salary 13%, Status 10%, Actions 17%);
  added `px="sm"` to Offboard/Activate button; `npm run build` clean;
...
- `money-transfer`: salary money transfer/payroll processing — compliance-heavy, banking concern.
  Out of scope for salary _data_ management.
