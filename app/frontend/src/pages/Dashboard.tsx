...
function KpiCard({
  ...
  return (
    <Card
      withBorder
      className="card-lift animate-fade-in-up"
      style={{ animationDelay: `${delay}ms`, animationFillMode: "both" }}
    >
      <Text size="sm" c="dimmed">
        {label}
      </Text>
      <Text size="xl" fw={700} mt={8} mb={4}>
        {value}
      </Text>
    </Card>
  );
  ...
  if (!data)
    return (
      <Container size="xl" py="xl">
        <div
          className="shimmer"
          style={{ height: 32, width: 240, marginBottom: 16 }}
        />
  ...
  return (
    <Container size="xl" py="xl">
      ...
      <SimpleGrid cols={{ base: 1, sm: 3 }} mb="xl">
        {[
          { label: "Headcount (active)", value: String(data.headcount) },
          {
            ...
            value: `$${Number(data.total_usd).toLocaleString()}`,
          },
              ...
              <Table.Tbody>
                {data.top_earners.slice(0, 5).map((t, i) => (
                  ...
                  >
                    ...
                    <Table.Td>
                      ${Number(t.usd_equivalent).toLocaleString()}
                    </Table.Td>
                  </Table.Tr>
                ))}
              </Table.Tbody>
      ...
      <SimpleGrid cols={{ base: 1, md: 2 }} mb="xl">
        ...
        >
          <Text fw={600} mb="sm">
            Avg by department (USD)
          </Text>
          {entries(data.avg_by_department).map(({ k, v }) => (
            <Text key={k} size="sm" mt={8} mb={4}>
